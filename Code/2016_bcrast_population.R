...
##Initialization: download data from https://www12.statcan.gc.ca/census-recensement/2011/geo/bound-limit/bound-limit-2016-eng.cfm
##and https://www12.statcan.gc.ca/census-recensement/2011/geo/ref/att-eng.cfm?year=2016

##Store unzipped data in /BCFireCube/Data/2016Census
...
bnd <- st_read("Data/2016Census/ldb_000b16a_e.shp")

#pull census population data
pop <- read.csv("Data/2016Census/2016_92-151_XBB.csv")
...
pop <- pop %>% mutate(DBUID = as.character(DBuid.IDidu), LANDAREA = DBarea2016.IDsup2016) %>% 
  select(c("DBUID", "DBpop2016.IDpop2016", "LANDAREA"))

#left join pop data to bnd
bnd <- bnd %>% left_join(pop, by = "DBUID")

#calculate average population density per km^2
bnd <- bnd %>% mutate(population = DBpop2016.IDpop2016/LANDAREA)
...
writeCDF(bnd, "Products/2016_bcrast_population.nc", varname="population", longname="Population per Square Kilometer", unit = "population/km^2")
