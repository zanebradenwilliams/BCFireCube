...
dwl <- read.csv("Data/2016Census/2016_92-151_XBB.csv")
...
dwl <- dwl %>% mutate(DBUID = as.character(DBuid.IDidu), LANDAREA = DBarea2016.IDsup2016) %>% 
  select(c("DBUID", "DBtdwell2016.IDtlog2016", "LANDAREA"))

#left join dwl data to bnd
bnd <- bnd %>% left_join(dwl, by = "DBUID")

#calculate average dwelling density per km^2
bnd <- bnd %>% mutate(dwellings = DBtdwell2016.IDtlog2016/LANDAREA)
...
writeCDF(bnd, "Products/2016_bcrast_dwellings.nc", varname="dwellings", longname="Dwellings per Square Kilometer", unit = "dwellings/km^2")
